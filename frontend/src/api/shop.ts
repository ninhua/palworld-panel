...
export interface ShopProduct extends ShopProductInput {
  id: string;
  created_at: string;
  updated_at: string;
}
...
const emptyOrderResult: ShopOrderResult = {
  order: emptyOrder,
  account: {
    player_uid: '',
    status: 'active',
...
export const shopApi = {
  ...
  products: (includeDisabled = true) => handleRequest<unknown, ProductListResult>(
    () => apiClient.get('/shop/products', { params: { include_disabled: includeDisabled, limit: 500 } }),
    { items: [], count: 0 },
    { fallbackOnError: false },
  ),
  createProduct: (input: ShopProductInput) => handleRequest<unknown, ShopProduct>(
    () => apiClient.post('/shop/products', input),
    emptyProduct,
    { fallbackOnError: false },
  ),
  updateProduct: (id: string, input: ShopProductInput) => handleRequest<unknown, ShopProduct>(
    () => apiClient.put(`/shop/products/${encodeURIComponent(id)}`, input),
    { ...emptyProduct, id },
    { fallbackOnError: false },
  ),
  archiveProduct: (id: string) => handleRequest<unknown, ShopProduct>(
    () => apiClient.delete(`/shop/products/${encodeURIComponent(id)}`),
  ...
  completeOrder: (id: string) => handleRequest<unknown, ShopOrderResult>(
    () => apiClient.post(`/shop/orders/${encodeURIComponent(id)}/complete`),
    emptyOrderResult,
    { fallbackOnError: false },
  ),
  cancelOrder: (id: string) => handleRequest<unknown, ShopOrderResult>(
    () => apiClient.post(`/shop/orders/${encodeURIComponent(id)}/cancel`),
    emptyOrderResult,
    { fallbackOnError: false },
  ),
};
