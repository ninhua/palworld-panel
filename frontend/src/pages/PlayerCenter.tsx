...
import {
  AlertTriangle, BookOpen, Boxes, CheckCircle2, Database, Gamepad2, Gift, LoaderCircle, MessageSquareText,
      ...
      <section className="grid min-h-[720px] overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm lg:grid-cols-[330px_minmax(0,1fr)]">
        ...
        <main className="min-w-0">
          {selected && overview ? <>
            ...
            {activeTab === 'profile' && <><PlayerOverview player={overview} progression={progressionQuery.data} savePals={savePalsQuery.data?.items ?? []} saveInventory={saveInventoryQuery.data?.containers ?? []} saveLoading={savePalsQuery.isLoading || saveInventoryQuery.isLoading} /><SaveInventoryPanel containers={saveInventoryQuery.data?.containers ?? []} catalog={catalogQuery.data?.items ?? []} loading={saveInventoryQuery.isLoading} /></>}
