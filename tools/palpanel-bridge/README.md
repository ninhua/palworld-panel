...
`PalPanelBridge-v0.1.57-ue4ss-c838a8ac.zip`, containing the complete
...
1. Extract `PalPanelBridge-v0.1.57-ue4ss-c838a8ac.zip`.
...
Version `0.1.48` removes the global object-array scan and uses bounded `FindAllOf`
queries for seven confirmed worker/assignment classes. This avoids long game-thread
stalls while retaining the targeted runtime metadata probe.
...
Version `0.1.51` accepts the runtime work-suitability input as a validated one-byte
enum property instead of requiring `FByteProperty`, and accepts either byte or int32
rank returns. This preserves the same bounded getter-only behavior.

Version `0.1.52` replaces `FindFirstOf("World")` with a bounded, targeted World-class
selection that requires a Pal main-world path, rejects generated grid sub-worlds, and
...
Version `0.1.55` retains that network path and adds a dedicated-server fallback when the
offline RPC produces no assignment. It uniquely binds a live `PalAIActionCompositeWorker`
...
Version `0.1.57` accepts a validated offline assignment even while the base worker's Pawn
and AI are dormant. The mutation reports `pending_activation` and keeps the exact guarded
request in the current server process. Every 30 seconds it retries one pending assignment;
as soon as base simulation creates the worker AI, the original job changes to `succeeded`
...
Call `GET /api/bases/:id/work-assignments` first to enumerate the existing persistent keys;
an empty list means this deliberately limited fallback cannot create a new assignment.
