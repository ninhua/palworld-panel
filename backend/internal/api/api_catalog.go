...
var apiCatalogExact = map[string]apiCatalogDescriptor{
	...
	"GET /api/bases/:id/work-assignments": {
		Category: "世界存档", Summary: "枚举持久工作设施与指派", Description: "只读解析当前 DedicatedServerName 对应的 Level.sav，即使 WorkAssignMap 为空也返回指定据点的持久工作设施键，并单独列出尚未证明据点归属的 Assign 记录。", Permission: "world:reset", Response: "当前世界、Level.sav SHA256、工作设施、Map 指派和未归属 Assign 诊断；无记录时数组为空。", Patched: true,
	},
