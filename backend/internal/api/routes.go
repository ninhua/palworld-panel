...
func (s Server) registerSystemRoutes(api *gin.RouterGroup) {
	...
	api.GET("/panel/update/status", s.panelUpdateStatus)
	api.POST("/panel/update/check", Require(PermServerControl), s.panelUpdateCheck)
	api.POST("/panel/update", Require(PermServerControl), s.panelUpdate)
	...
	api.GET("/system/diagnostics", RequireDiagnosticHTTPAdmin(), s.diagnosticStatus)
	api.POST("/system/diagnostics/http", RequireDiagnosticHTTPAdmin(), s.runDiagnosticHTTP)
	api.POST("/system/diagnostics/shell", RequireInteractiveAdmin(), s.runDiagnosticShell)
	api.GET("/system/diagnostics/support-bundles/status", RequireInteractiveAdmin(), s.supportBundleStatus)
	api.GET("/system/diagnostics/support-bundles", RequireInteractiveAdmin(), s.listSupportBundles)
	api.POST("/system/diagnostics/support-bundles", RequireInteractiveAdmin(), s.createSupportBundle)
	api.GET("/system/diagnostics/support-bundles/:id/download", RequireInteractiveAdmin(), s.downloadSupportBundle)
	api.DELETE("/system/diagnostics/support-bundles/:id", RequireInteractiveAdmin(), s.deleteSupportBundle)
	...
	api.POST("/incidents/:id/ack", Require(PermServerControl), s.incidentAction("acknowledged"))
	api.POST("/incidents/:id/resolve", Require(PermServerControl), s.incidentAction("resolved"))
	api.POST("/incidents/:id/reopen", Require(PermServerControl), s.incidentAction("open"))
	api.POST("/system/incidents/webhook/test", RequireInteractiveAdmin(), s.testIncidentWebhook)
...
func (s Server) registerContentRoutes(api *gin.RouterGroup) {
	...
	api.GET("/config/palworld/revisions", Require(PermConfigWrite), s.listPalworldConfigRevisions)
	api.GET("/config/palworld/revisions/:id/diff", Require(PermConfigWrite), s.getPalworldConfigRevisionDiff)
	api.POST("/config/palworld/revisions/:id/restore", Require(PermConfigWrite), Require(PermServerControl), s.restorePalworldConfigRevision)
...
func (s Server) registerSecurityRoutes(api *gin.RouterGroup) {
	...
	api.POST("/security/paldefender/gm/commands/runtime", Require(PermSecurityWrite), s.palDefenderGMRCONCommands)
