...
- 面向用户的事件必须由两份结构化快照推断并显式标记 `inferred=true`；不得把“获得帕鲁”伪装成可证明的捕获日志，也不得声称知道快照之间的精确发生时间和原因。
- 语义事件至少覆盖玩家等级/公会、按玩家或据点归属的物品增减、帕鲁获得/丢失/转移/成长、据点建筑/工作帕鲁/存储/状态/坐标和公会成员/据点/会长变化；原始字段差异必须继续保留用于审计。
...
- 主机角色迁移必须通过后台生命周期任务执行；HTTP 请求只负责预检和提交，禁止在反向代理连接内同步完成停服、复制和重启。
