...
class VendorCtlTest(unittest.TestCase):
    ...
    def test_init_verify_and_tamper_detection(self):
        with tempfile.TemporaryDirectory() as temp:
            ...
            for option, name in (("--palops-web", "palops"), ("--maplibre", "maplibre"), ("--palcalc", "palcalc"), ("--uesave", "uesave")):
                args += [option, self.make_tree(base, name)]
            self.run_tool(*args)
            manifest = json.loads((mirror / "vendor-manifest.json").read_text())
            self.assertEqual({item["id"] for item in manifest["components"]}, {"palops-web", "maplibre-gl", "palcalc", "uesave"})
            ...
            (mirror / "sources/palops-web/file.txt").write_text("changed", encoding="utf-8")
    ...
    def test_symlink_input_is_rejected(self):
        with tempfile.TemporaryDirectory() as temp:
            ...
            result = self.run_tool("init", "--root", base / "mirror", "--lock", LOCK, "--palops-web", source, check=False)
            self.assertNotEqual(result.returncode, 0)
