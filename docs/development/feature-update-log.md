...
- 首次 CI `32692632502` 的业务失败仅为新增路由未登记 OpenAPI，已补齐两阶段接口契约；
  同次漏洞门禁检测到 Go 1.25.12 标准库及 `x/net` 的新公告，最小升级到 Go 1.25.13 与
  `x/net` 0.55.0，不扩展业务修改。
- 第二次 CI `32693460687` 的构建与 Go 漏洞检查已通过，唯一失败为前端间接依赖
  `nanoid` 的新安全公告；锁文件最小升级至 3.3.18，未修改前端业务代码。
- 第三次 CI `32694675368` 全部成功；Windows 包 SHA256 为
  `a15f07aa273ebc1692770a92cb878f44ab4ddcef6838860d9b8870b1aedc654f`，包内后端、
...
- 实机结果：Action `32455861478`、SHA256
  `883b59d5346d7d4b7cd571d727e15ad5d92464319de2cd9fbcdbc485f7d8cebe` 已由脚本部署；
  `2026-08-21T14:50:04.287+08:00` 健康检查返回 0.1.57。无人在线时任务
  `mutation_1787295036917_3` 对已通过预检的 `PalWorkProgress` 返回
  `pending_activation`，经过一个 30 秒自动重试周期仍可查询、服务器稳定且未误报成功。
  这确认面板调用本身已完全不要求玩家在线；休眠据点没有 Pawn/AI 时，实际工作变更会在
  据点模拟激活后由插件自动落地并回读确认。
