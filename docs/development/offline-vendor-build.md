...
`palpanel-map-assets` 必须是 `ninhua/palpanel-assets` 的完整快照，包含三语言 POI、两张地图的完整瓦片、图标、元数据和许可证。
...
`--palops-web` 和 `--map-tiles` 暂时保留为兼容别名，但现在都要求传入完整的 `palpanel-assets` 根目录，不再接受拆分的 POI 与瓦片输入。
...
scripts/package.sh --version v1.3.0-custom.0.8.43 --clean

# 完全离线，要求地图资源、源码和四类缓存完整
...
- `palpanel-map-assets` 快照必须保留资源仓库中的许可证和来源说明。
