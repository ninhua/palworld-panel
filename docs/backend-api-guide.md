...
诊断接口分为低风险状态/内网 HTTP 测试和高风险主机能力。状态与 HTTP 测试接受已登录管理员的同源浏览器会话或管理员 Bearer API Key；普通操作员和查看者 API Key 会被拒绝。Shell 与支持包仍只接受管理员会话，API Key 始终被拒绝。

权限矩阵：

| 凭据 | 状态 | HTTP 测试 | Shell/支持包 |
| --- | --- | --- | --- |
| 管理员 session | 允许 | 允许 http/https、GET/HEAD/POST/PUT/PATCH/DELETE | 允许（仍需现有开关/确认） |
| 管理员 API Key | 允许 | 仅允许 `http`、GET/HEAD/POST | 拒绝 |
| operator/viewer API Key | 拒绝 | 拒绝 | 拒绝 |
...
session 仅允许 `http`、`https` 和回环/私网目标；管理员 API Key 进一步仅允许 `http` 和 GET/HEAD/POST。所有凭据都禁止公网目标、URL 用户信息及代理相关请求头。最多跟随 3 次且每次仍满足私网限制；单次最长 15 秒，请求体和响应体最多 64 KiB。残余风险是管理员 API Key 仍可触发受限内网 HTTP 请求，因此应像其他高权限凭据一样短期、可撤销并避免通过公网明文传输。
