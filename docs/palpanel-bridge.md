...
`0.1.41` 在严格校验 `FGuid -> PalBaseCampModuleResourceCollectWorkInfo` ABI 后，有界读取
`MapObjectWorkInfoMap`，每项输出 `map_object_id` 与内部 `work_id`；同时枚举最多 64 个
已加载 `PalWorkBase` 类样本的相关属性和函数 ABI，用于把 WorkId 对应到实际工作对象。
本版仍不调用设施使用、生产或帕鲁分配函数，不改变据点状态。

`0.1.42` 不再只按类保留一个工作样本，而是最多读取 128 个已加载工作实例；严格校验
...
`0.1.44` 继续保留上述据点工作，同时对其他已加载工作只在分配信息或已分配角色数量
非零时输出，用于发现实际工作台/设施的活动分配，不恢复全世界空工作对象列表。
...
`0.1.47` 进一步要求候选完整路径属于 `/Game/`，排除 `/Script` 下的 Enum、Function、
DelegateFunction 和 ScriptStruct 定义，目标是取得世界中的实际 Worker/Assign 实例。
...
先用 `GET` 只读枚举 `work_bases` 中的持久设施键和 `assignments` 中的已有指派；即使
`WorkAssignMap` 为空仍可列出设施。空指派数组表示当前安全版本不会凭空创建指派。
`unscoped_assignments` 仅诊断独立的 `EPalWorkableType::Assign` 记录；在没有据点归属证据
前不会把它用于写入。
