...
`0.8.64` 建立 Boss 模板、奖励配置和手动召唤审计账本；`0.8.66` 增加 `/boss` 管理界面。当前版本只记录召唤请求和状态，不会向 PalDefender 或 RCON 发送生成命令。
...
`/boss` 页面直接复用上述 API，并读取 PalDefender 的本地目录：

- 奖励物品从物品目录选择，面板生成 `items` 数组；
- 奖励帕鲁从已保存模板选择，面板生成 `pal_templates` 数组；
- Boss 的 `pal_id` 从帕鲁目录搜索选择；
- 召唤状态、执行模式和审计事件在界面中显示为中文。

目录选择只负责生成合法配置，不改变 `record_only` 的执行限制。
